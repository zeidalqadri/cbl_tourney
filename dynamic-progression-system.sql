...
-- 1. Create progression rules table
...
-- 2. Insert progression rules for current tournament
...
ON CONFLICT (source_match, source_position) DO NOTHING;

-- 3. Create function to advance teams
...
-- 5. Function to manually process all completed matches
...
-- 6. Create view to see progression status
...
-- 7. Execute initial progression processing
SELECT * FROM process_all_completed_matches();

-- 8. Show current knockout stage status
SELECT * FROM tournament_progression 
WHERE match_number IN (145,146,147,148,149,150,245,246,247,248,249,250)
ORDER BY match_number;